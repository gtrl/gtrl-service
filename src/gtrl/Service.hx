...
class Service {
	...
	static function start(configFile:String, setupFile:String) {
		...
		return Promise.all([Json.readFile(configFile), Json.readFile(setupFile)]).then(function(r) {
			...
			switch config.net.host {
				case null, 'auto':
					config.net.host = om.Network.getLocalIP()[0];
				default:
			}
	...
	public static function log(msg:String, ?time:Date) {
		if (time == null)
			time = Date.now();
		if (!isSystemService)
			print(DateTools.format(time, "%H:%M:%S") + ' ');
		println(msg);
	}
	...
	static function main() {
		...
		var argsHandler:{getDoc:Void->String, parse:Array<Dynamic>->Void} = null;
		argsHandler = hxargs.Args.generate([@doc("Backup log.db")
			["backup", "bak"] => function(?dir:String) {
				if (dir == null)
					dir = 'dat';
			...
			["--config", "-c"] => function(?path:String) {
				exitIf(!FileSystem.exists(path), 'Config file [$path] not found');
				configFile = path;
			}, @doc("Path to setup file")
			["--setup", "-s"] => function(path:String) {
				exitIf(!FileSystem.exists(path), 'Setup file [$path] not found');
				setupFile = path;
			},
			["--service"] => function() isSystemService = true, @doc("Print usage")
			["--help", "-help", "-h"] => function() {
